...
class SpidSpMetadataCheckPublic(object):
    def test_Contacts_PubPriv(self, contact_type="other", entity_type=""):
        ...
        if entity_desc:
            ...
            self._assertTrue(
                ...
                "The contactType attribute MUST have a value", **_data
            )
            self._assertTrue(
                entity_desc[0].get("contactType") == contact_type,
                f'The contactType must be "{contact_type}"',
                description=entity_desc[0].get("contactType"), **_data
            )
        ...
        if not entity_type:
            self._assertTrue(
                ...
                f'"{contact_type}" MUST be present', **_data
            )
    ...
    def test_Extensions_PubPriv(self):
        ...
        for cont in _conts:
            ...
            self._assertFalse(
                ...
                description=etree.tostring(cont).decode(), **_data
            )
    ...
    def test_contactperson_phone(self, phone_xpath="//ContactPerson/TelephoneNumber"):
        ...
        if phone:
            ...
            self._assertTrue(
                phone,
                f"The {phone_xpath} element MUST have a value", **_data
            )
            self._assertTrue(
                (" " not in phone),
                f"The {phone_xpath} element MUST not contain spaces",
                description=phone, **_data
            )
            self._assertTrue(
                (phone[0:3] == "+39"),
    ...
    def test_Contacts_IPACode(self):
        ...
        if self.production:
            ...
            self._assertTrue(
                ipacode, "The IPACode element MUST be present", **_data
            )
            if ipacode:
                ipacode = ipacode[0]
                self._assertTrue(
                    ipacode.text,
                    "The IPACode element MUST have a value", **_data
                )
                self._assertTrue(
                    ipacode.text,
                    "The IPACode element MUST have a value", **_data
    ...
    def test_extensions_public_private(self, ext_type="Public", contact_type="other"):
        ...
        self._assertTrue(
            ...
            "this element MUST be present", **_data
        )
        if ctype:
            self._assertFalse(
                ctype[0].text,
                f"The {ext_type.title()} element MUST be empty", **_data
            )

        ctype = self.doc.xpath(f"//ContactPerson/Extensions/{ext_type_not.title()}")
        self._assertFalse(
            ctype,
            f"The {ext_type_not.title()} element MUST not be present", **_data
        )
        return self.is_ok(_method)
    ...
    def test_Contacts_VATFC(self, private=False):
        ...
        if vats:
            self._assertTrue(
                vats[0].text,
                "The VATNumber element MUST have a value", **_data
            )
            self._assertTrue(
        ...
        if fcs:
            ...
            self._assertTrue(
                fc.text,
                "The FiscalCode element MUST have a value", **_data
            )
    ...
    def test_extensions_cie(self, ext_type="Public"):
        ...
        for contact in contacts:
            for ele in attrs:
                ...
                self._assertTrue(
                    ctype,
                    f"{ele} element MUST be present", **_data
                )

                # is <= because already protected with the previous check
                self._assertTrue(
                    (len(ctype) <= 1),
                    f"only one {ele} element MUST be present", **_data
                )

                if ctype:
                    self._assertTrue(
                        ctype[0].text,
                        f"The {ele} element MUST have a value", **_data
                    )
